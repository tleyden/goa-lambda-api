...
//
// API "HelloServerlessGoa": CLI Commands
//
// Command:
// $ goagen
...
func (cmd *ShowHelloCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/hello/%v", url.QueryEscape(cmd.WhatToSay))
	}
...
func (cmd *ShowHelloCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	...
	cc.Flags().StringVar(&cmd.WhatToSay, "whatToSay", whatToSay, `What To Say Hello To`)
}
